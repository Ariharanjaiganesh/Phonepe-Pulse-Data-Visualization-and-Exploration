...
class agg_users:
    ...
    def aggregated_users(self, state, year, quarter, path):
        ...
        if urs_df:
            for i in urs_df:
                all_rows = {"Device_Brand": i['brand'], "Brand_count": i['count'], "percentage": i['percentage'],
                            "Registered_users": reg_users, "App_opening": app_opens, "state": state, "year": year,
                            "quarter": quarter}
                all_rows_df = pd.DataFrame.from_dict([all_rows])
...
class map_tran:
    ...
    def map_transaction(self, state, year, quarter, path):
        ...
        if map_dft:
            for i in map_dft:
                all_rows = {"Distric_Name": i['name'], "Total_Transaction_count": i['metric'][0]['count'],
                            "Total_Transaction_amount": i['metric'][0]['amount'], "state": state, "year": year,
                            "quarter": quarter}
                all_rows_df = pd.DataFrame.from_dict([all_rows])
                self.state_map_transaction = pd.concat([self.state_map_transaction, all_rows_df])
                self.state_map_transaction['Total_Transaction_amount'] = self.state_map_transaction[
                    'Total_Transaction_amount'].apply(lambda x: int(x))
...
class map_usr:
    ...
    def map_users(self, state, year, quarter, path):
        ...
        if map_dft:
            for i in map_dft:
                all_rows = {"Distric_Name": i, "Registered_users": map_dft[i]['registeredUsers'],
                            "App_opening": map_dft[i]['appOpens'], "state": state, "year": year, "quarter": quarter}
                all_rows_df = pd.DataFrame.from_dict([all_rows])
